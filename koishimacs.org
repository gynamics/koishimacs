...
#+begin_src emacs-lisp
  (use-package keycast
    ...
    (keycast-mode-line-insert-after 'violet-line-modes)
    )
#+end_src
...
Actually, this framework is too powerful and there has been a lot of extensions. I will just use some basic features it seems to have.
...
#+begin_src emacs-lisp
  (use-package popwin
    ...
    (define-advice company-show-doc-buffer (:around (f &optional r))
      ;; do not break company doc buffer
  ...
  (use-package vterm
    ...
    (defun my:follow-vterm-cwd ()
      "Make emacs follow vterm's current work directory."
      (interactive)
      (cd (file-truename
  ...
  (use-package dashboard
    ...
    (dashboard-navigator-buttons
     '(((" " "Agenda" "Task for this week"
         ...
         warning "[" "]")
        ("󰾱 " "Gnus" "Browse News & Mails"
         (lambda (&rest _) (gnus))
         warning "[" "]")
        (" " "News" "Browse Feeds"
         (lambda (&rest _) (newsticker-show-news))
         warning "[" "]")
...
#+begin_src emacs-lisp
  (use-package yasnippet
    ...
    :hook ((my/org-mode my/prog-mode sgml-mode nxml-mode) . yas-minor-mode)
...
#+begin_src emacs-lisp
  (use-package company
    ...
    (defvar my/company-cascade-table
      ...
      "Each item is a CONS, whose CAR is a symbol or list, CDR is a list of backends.")
    ...
    (defun my:select-company-backends ()
      (let ((selected
             (cons
              ...
              (mapcan
               (lambda (cs)
                 (let ((p (lambda (x) (eval `(bound-and-true-p ,x))))
                       (a (car cs)))
                   (when (or (derived-mode-p a)
                             (if (listp a) (cl-some p a) (funcall p a)))
                     (list (cdr cs)))))
    ...
    (setq company--electric-commands
          '(scroll-other-window scroll-other-window-down mwheel-scroll
            windmove-up windmove-down windmove-left windmove-right
            other-window mouse-drag-region mouse-set-point))
    :custom
    ...
    ((inferior-octave-mode maxima-inferior-mode slime-repl-mode geiser-repl-mode)
     . company-mode)
...
#+begin_src emacs-lisp
  ...
  (use-package dimmer
    ...
    :when my/load-gui-config-p
    :init
    (defun my:pdf-view-mode-p ()
      (bound-and-true-p pdf-view-mode))
    :custom
    (dimmer-mode t)
    ...
    (dimmer-prevent-dimming-predicates
     '(window-minibuffer-p my:pdf-view-mode-p))
    )
  ...
  (use-package rainbow-mode
    ...
    :hook ((emacs-lisp-mode sgml-mode css-mode js-base-mode conf-mode nxml-mode yaml-ts-mode) . rainbow-mode)
    )
...
#+begin_src emacs-lisp
  ...
  (use-package flycheck
    ...
    (emacs-lisp-mode
     . (lambda ()
         (cond
          ((member (buffer-name) '("*Pp Eval Output*" "*Pp Macroexpand Output*"))
           (flycheck-mode -1)))))
    (c-mode . (lambda () (setopt flycheck-cppcheck-checks '("performance"))))
    (rust-mode . (lambda () (setopt flycheck-rust-crate-type nil)))
    :bind
  ...
  (use-package eldoc
    ...
    (when (bound-and-true-p popwin-mode)
      (defun my:popwin:eldoc ()
        (interactive)
        (unless (buffer-live-p eldoc--doc-buffer) (eldoc))
        (popwin:popup-buffer (eldoc-doc-buffer)))

      (keymap-set popwin:keymap "d" #'my:popwin:eldoc))
    )
...
#+begin_src emacs-lisp
  (use-package format-all
    ...
    :bind
    (:map prog-mode-map
          ("C-c TAB" . format-all-region-or-buffer)
          )
    )

  (use-package cmacexp
    :commands (my:c-macro-expand)
    :config
...
** project management
Since Emacs 28, Emacs has integrated ~project.el~ for project management (its keymap is bound to ~C-x p~ by default). So we don't need projectile anymore.

Currently, ~git~ is the most popular version control tool, and ~magit~ provides the best interface for it.
...
#+begin_src emacs-lisp
  ...
  (use-package gnu-apl-mode
    :ensure t
    :init
    ;; exchange key prefix to 96 (backquote), which is similar to dyalog
    (with-eval-after-load 'gnu-apl-input
      (customize-set-variable 'gnu-apl-key-prefix ?`))
    (with-eval-after-load 'gnu-apl-symbols
      (setcdr (assoc "backslash-bar" gnu-apl--symbols) '("⍀" "."))
      (setcdr (assoc "delta-underbar" gnu-apl--symbols) '("⍙" ">")))
    :hook ((gnu-apl-mode gnu-apl-interactive-mode)
  ...
  (use-package tex
    :ensure auctex
    :config
    (if (featurep 'pdf-tools-autoloads)
        (progn
          (setf (alist-get 'output-pdf TeX-view-program-selection) '("PDF Tools"))
          (add-hook 'TeX-after-compilation-finished-functions #'TeX-revert-document-buffer)
          )
      (setf (alist-get 'output-pdf TeX-view-program-selection) '("Okular")))
    :hook
  ...
  (use-package pyvenv
    ...
    :diminish " "
    :init
    (defun my:parent-directories (path)
      "Return a list of all parent directories of PATH in bottom-up order."
      (when-let* ((parent (file-name-parent-directory path)))
        (cons parent (my:parent-directories parent))))

    (defun my:find-python-venv (path)
      (seq-find (lambda (n)
                  (when (file-exists-p n)
                    (file-exists-p (file-name-concat path n))))
                '("venv" "virtualenv")))
    :hook
    (python-mode
     . (lambda ()
         ...
         (when-let*
             ((f (buffer-file-name))
              (d (seq-find #'my:find-python-venv (my:parent-directories f))))
           (pyvenv-activate (concat d (my:find-python-venv d))))))
    )
  ...
  (use-package slime
    ...
    :config
    ;; fontify the document buffer with font lock from `slime-company'
    (define-derived-mode slime-fancy-doc-mode help-mode "Slime Doc"
      "Fancy documentation mode for slime."
      (setq font-lock-defaults
            '((("^#<\\([A-Z]+\\)\\b" 1 'font-lock-keyword-face)
               ("^[ \t]*\\(\\[[ a-z]+\\]\\)" 1 'font-lock-comment-face)
               ("^\\([A-Z][^:]+\\):\\([ \t]\\|$\\)" 1 'font-lock-doc-face)
               ("(\\(FUNCTION\\|VALUES\\|OR\\|EQL\\|LAMBDA\\)\\b" 1 'font-lock-keyword-face)
               ("[ \t(]\\(&[A-Z0-9\\-]+\\)\\b" 1 'font-lock-type-face)
               ("[ \t(]\\(:[A-Z0-9\\-]+\\)\\b" 1 'font-lock-builtin-face)
               ("[ \t(]\\(T\\|t\\|NIL\\|nil\\|NULL\\|null\\)\\b" 1 'font-lock-constant-face)
               ("\\b[+-]?[0-9/\\.]+[sdeSDE]?+?[0-9]*\\b" 0 'font-lock-number-face)
               ("#[xX][+-]?[0-9A-F/]+\\b" 0 'font-lock-number-face)
               ("#[oO][+-]?[0-7/]+\\b" 0 'font-lock-number-face)
               ("#[bB][+-]?[01/]+\\b" 0 'font-lock-number-face)
               ("#[0-9]+[rR][+-]?[0-9A-Z/]+\\b" 0 'font-lock-number-face)))))

    (define-advice slime-show-description (:after (&rest r))
      (with-current-buffer (slime-buffer-name :description)
        (slime-fancy-doc-mode)))
    :hook
  ...
  (use-package auto-rename-tag
    ...
    :diminish " 󰅴"
    :hook
    ((nxml-mode sgml-mode) . auto-rename-tag-mode)
    )
  ...
  (use-package geiser
    ...
    ;; press C-c C-s to switch implementation
  ...
  ;; Maxima's built-in emacs plugin has been deprecated, use this newer one
...
#+begin_src emacs-lisp
  (use-package org-roam
    ...
    (defun my:org-roam-consult-grep ()
      ...
      (interactive)
      (dlet ((grep-find-ignored-directories (list "ltximg/")))
  ...
  (use-package org-roam-ql
    :ensure t
    :config
    (setq org-roam-ql-preview-function
          (lambda (node _)
            (if (= (org-roam-node-level node) 0)
                ;; if it is at top level, insert all its content
                (let ((beg (save-excursion
                             (org-roam-end-of-meta-data t)
                             (point)))
                      (end (point-max)))
                  (string-trim (buffer-substring-no-properties beg end)))
              ;; otherwise simply call the default function.
...
#+begin_src emacs-lisp
    ...
    (advice-add
     ...
     (lambda ()
       (when-let* ((f (buffer-file-name)))
         (concat "./" (file-name-base f) ".assets"))))
    :custom
  ...
  ;; fontify code blocks as it is when use `org-export-html'
...
#+begin_src emacs-lisp
  ...
  (use-package ellama
    ...
    ;; Here we use ollama + qwen3.6 as default configuration.
    (require 'llm-ollama)
    (let ((m (make-llm-ollama
              :chat-model "qwen3.6"
              :embedding-model "qwen3-embedding:latest"
              :default-chat-non-standard-params '(("num_ctx" . 262144)))))
      (setopt ellama-provider m))
  ...
  (use-package pdf-tools
    ...
    ;; support display page number in violet line
    (pdf-view-mode
     . (lambda () (setq violet-line-position mode-line-position)))
    )
...
#+begin_src emacs-lisp
  (use-package package-lint
    ...
    :init
    (defun my:package-lint-file (&optional filename)
      "Display lint errors and warnings for a file of FILENAME."
      (interactive
       (list (read-file-name "Lint file: " nil (buffer-file-name))))
...
#+begin_src emacs-lisp
  ...
  (use-package toc-glue
    ...
    :mode
    ("\\.toc\\'" . toc-glue-toc-mode)
    ("\\.sbm.el\\'" . toc-glue-mode)
    ("\\.bm.el\\'" . toc-glue-djvu-mode)
    )
