...
#+begin_src emacs-lisp
  ...
  (use-package treesit
    ...
    ;; enable *-ts-mode by default for those languages without *-mode support
  ...
  (use-package c-ts-mode
    ...
    (defun my:c-ts-set-style (&optional style)
      ...
      (interactive
       (list (completing-read
              "Style: " (mapcar #'car (c-ts-mode--indent-styles major-mode)))))
      ;; detect file mode with cc-mode
      (if style
          (c-ts-mode-set-style style)
        ;; else detect style with `c-guess-buffer'
        (let ((is-c++-p (eq major-mode 'c++-ts-mode))
              (guessed-offset))
          (unless (bound-and-true-p c-ts-set-style--reentry)
            (if (or is-c++-p
                    (save-excursion
                      (save-restriction
                        (save-match-data
            ...
            (setq guessed-offset
                  (condition-case nil
                      (c-guess-buffer)
                    (:success c-guess-guessed-basic-offset)
                    ;; catch error if guess failed
                    (error (message "Guess failed, fallback to 2.") 2)))
            ;; mask this hook function when reentry ts mode
            (dlet ((c-ts-set-style--reentry t))
              (if is-c++-p (c++-ts-mode) (c-ts-mode)))
            (setq-local c-ts-mode-indent-offset guessed-offset))))
      ;; refresh indent bars
    ...
    :hook
    ((c-ts-mode c++-ts-mode) . my:c-ts-set-style)
    )
  ...
  (use-package pyvenv
    ...
    (python-mode
     . (lambda ()
         (pyvenv-mode)
         (when-let*
             ((d (seq-find #'my:find-python-venv
                           (my:parent-directories (buffer-file-name)))))
...
#+begin_src emacs-lisp
  (use-package org
    ...
    (setf
     (alist-get 'dvisvgm-pdf org-preview-latex-process-alist)
...
#+begin_src emacs-lisp
  ...
  (use-package ellama
    ...
    (require 'llm-ollama)
    ;; Here we use Qwen3-30B-A3B MoE model with default configuration.
    (setopt ellama-provider
            (make-llm-ollama
             :chat-model "alibayram/Qwen3-30B-A3B-Instruct-2507:latest"
