...
To accelerate emacs startup we do lazy loading. This feature can be enabled automatically by setting ~use-package-deferring-keywords~, all configurations with these keywords present will be deferred by default.
...
#+begin_src emacs-lisp
  ...
  ;; involve this to make use-package find personal-keybindings
...
#+begin_src emacs-lisp
  ...
  If BYTE-COMPILE-P is given as t, byte compile it."
    (interactive "P")
    (let ((config-org (concat user-emacs-directory "koishimacs.org"))
          (config-el (concat user-emacs-directory "koishimacs.el")))
...
#+begin_src emacs-lisp
  ...
  (use-package register-list
    :ensure t
    :functions (my:popwin:register-list)
    :autoload (register-list-refresh)
    :preface
...
#+begin_src emacs-lisp
  (use-package dashboard
    ...
    (dashboard-footer            "Richard Stallman is proud of you.")
...
#+begin_src emacs-lisp
  ...
  (use-package auto-yasnippet
    ...
    :after yasnippet
    :bind
    ;; use the same prefix C-c &
    (:map yas-minor-mode-map
          ("C-c & w" . aya-create)
          ("C-c & TAB" . aya-expand)
          ("C-c & SPC" . aya-expand-from-history)
          ("C-c & d" . aya-delete-from-history)
          ("C-c & c" . aya-clear-history)
          ("C-c & n" . aya-next-in-history)
          ("C-c & p" . aya-previous-in-history)
...
#+begin_src emacs-lisp
  ...
  (use-package highlight-escape-sequences
    :ensure t
    :hook ((prog-mode org-mode) . hes-mode)
    )
...
Load ~magit~ configuration after ~windower~ to void keybinding conflicts with [[moving][moving:windower]].
...
#+begin_src emacs-lisp
  (use-package org
    ...
    (org-format-latex-options '(:foreground default :background default :scale 1.5 :html-foreground "Black" :html-background "Transparent" :html-scale 1.0 :matchers ("begin" "$1" "$" "$$" "\\(" "\\[")))
...
#+begin_src emacs-lisp
  ...
  (use-package go-translate
    ...
    :autoload (gt-start
               ...
               gt-translator
               gt-plz-http-client)
    :functions (my:gt-do-translate-quickly)
    :preface
