...
#+begin_src emacs-lisp
  (use-package company
    ...
    (when my/load-gui-config-p
      ...
      ;; This can be moved to :custom for emacs-31, which supports childframe in terminal
...
#+begin_src emacs-lisp
  ...
  (use-package indent-bars
    ...
    ;; not compatible with whitespace-mode
    (define-advice whitespace-mode (:before (&optional arg))
      (when (and (bound-and-true-p indent-bars-mode)
                 (not (bound-and-true-p whitespace-mode)))
        (setq-local ws-override-indent-bars t)
        (indent-bars-mode -1)))
    (define-advice whitespace-mode (:after (&optional arg))
      (when (and (not (bound-and-true-p whitespace-mode))
                 (bound-and-true-p ws-override-indent-bars))
        (setq-local ws-override-indent-bars nil)
        (indent-bars-mode)))
    :custom
  ...
  (use-package rainbow-mode
    ...
    :hook ((emacs-lisp-mode sgml-mode css-mode js-base-mode conf-mode nxml-mode yaml-pro-mode web-mode) . rainbow-mode)
    )
    ...
    (define-derived-mode slime-fancy-doc-mode help-mode "Slime Doc"
      ...
      (setq font-lock-defaults
            '((("^\\([^ ]\\{4,\\}\\)\\b" . (1 font-lock-function-name-face t))
               ...
               ("^\\([A-Z][A-Za-z ]+:\\)\\([    ]\\|$\\)" . (1 font-lock-doc-face t))
               ...
               ("\\b\\([A-Z0-9:+%<>#*\\.\\-]\\{2,\\}\\)\\b" . (1 font-lock-variable-name-face))))))
...
#+begin_src emacs-lisp
  (use-package org
    ...
    :config
    ;; xelatex is not very efficient, but only pdflatex supports tikz
    ...
    (setq org-preview-latex-default-process 'xelatex-pdf)
...
#+begin_src emacs-lisp
  (use-package ox
    :commands (my:blog-sitemap-update my:blog-rsync my:blog-export my:org-blog-post)
    :config
    ...
    (defun my:blog-sitemap-update ()
      "Update sitemap.html according to index.json."
      (interactive)
      (let* ((f (file-name-concat
                 my/org-blog-dir my/org-blog-post-prefix "index.json"))
             (data))
            ...
            (list
             ...
             (format "<span class=\"tag-list\"> &nbsp; %s &nbsp; </span>"
                     (mapconcat
                      ...
                      (gethash "tags" v)
                      ""))
             (format "<span class=\"timestamp item-date\">%s</span>"
                     (gethash "date" v))
             "</li>\n")
            data))
...
#+begin_src emacs-lisp
  ...
  (use-package oboe
    ...
    :custom
    (oboe-default-display-method 'display-buffer)
    :bind
