  ...
  (use-package company
    ...
    (defun my:polymeric-company-backends (backends)
      (cl-loop
       ...
       if pos
       collect (cl-subseq backend 0 pos) into ctx-aware
       and collect (cl-subseq backend (1+ pos)) into ctx-agnostic
       else
       collect (cl-subseq backend 0) into ctx-aware
       finally return
       (cl-remove-duplicates
        (apply #'append
               (append ctx-aware '((:with)) ctx-agnostic)))))
...
#+begin_src emacs-lisp
  ...
  (use-package paren
    ...
    :custom-face (show-paren-match-expression ((t (:background "#6E647D"))))
