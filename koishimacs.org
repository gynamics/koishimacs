...
#+begin_src emacs-lisp
  ...
  (use-package c-ts-mode
    :commands (c-ts-set-style)
    :config
    (require 'cc-mode)
    (defun c-ts-set-style (&optional style)
      "Detect and setup C indentation STYLE for `c-ts-mode'."
      (interactive (list (completing-read "Set to style: " c-style-alist)))
      ;; detect file mode with cc-mode
      (if style
          (progn
            (if (member (intern style) '(gnu k&r bsd linux))
                (c-ts-mode-set-style style)
              (c-set-style style))
            (setq-local c-ts-mode-indent-offset
                        (assoc 'c-basic-offset (assoc style c-style-alist))))
        ...
        (unless (bound-and-true-p c-ts-set-style--reentry)
          ...
          (dlet ((c-ts-set-style--reentry t))
            (c-or-c++-ts-mode))
          (setq-local c-ts-mode-indent-offset c-guess-guessed-basic-offset)))
      ;; refresh indent bars
      (when (bound-and-true-p indent-bars-mode)
        (indent-bars-reset)))
    :hook
    ((c-ts-mode c++-ts-mode) . c-ts-set-style)
    )
